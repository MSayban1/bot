...
app.get('/trigger', async (req, res) => {
    log('⚠️ Manual trigger received via HTTP request.');
    await runBot();
    res.json({ message: "Bot run triggered manually. Check logs below.", logs: logs.slice(-10) });
});

// --- Root Fallback ---
app.get('/', (req, res, next) => {
    // If static file middleware didn't find index.html, this runs
    res.send(`
        <h1>🤖 Auto Newsletter Bot is Running!</h1>
        <p>If you see this, the server is active.</p>
        <p>Go to <a href="/trigger">/trigger</a> to force the bot to run right now.</p>
        <p>Go to <a href="/api/data">/api/data</a> to see raw status.</p>
    `);
});
